<template>
  <div class="dashboard">
    <header class="header">
      <div>
        <h1 style="margin: 0; color: var(--text-main);">Gestión de Categorías</h1>
        <p style="margin: 0.2rem 0 0; color: var(--text-muted);">Administración de Tipos de Solicitud y Clasificación de Casos</p>
      </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="categoria in categorias" :key="categoria.id">
            <td style="font-weight: 600; color: var(--text-main);">{{ categoria.nombre }}</td>
            <td>
              <span :class="categoria.departamento_destino === 'HELPDESK' ? 'badge badge-info' : 'badge badge-secondary'">
                {{ categoria.departamento_destino === 'HELPDESK' ? 'HELPDESK' : 'DATA' }}
              </span>
            </td>
            <td style="font-size: 0.85rem; color: var(--text-muted);">{{ formatDate(categoria.created_at) }}</td>
            <td style="text-align: right;">
              <button class="btn btn-danger-link btn-sm" @click="confirmDelete(categoria)">
                🗑️ Eliminar
              </button>
            </td>
          </tr>
          <tr v-if="categorias.length === 0">
            <td colspan="4" class="text-center" style="padding: 3rem; color: var(--text-muted); text-align: center;">
              No hay categorías registradas en este departamento.
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <!-- Lista Móvil (< 768px) -->
    <div class="mobile-categorias-list hidden-desktop">
      <div v-for="categoria in categorias" :key="categoria.id" class="categoria-card card">
        <div class="categoria-card-header">
          <strong class="categoria-name">{{ categoria.nombre }}</strong>
          <span :class="categoria.departamento_destino === 'HELPDESK' ? 'badge badge-info' : 'badge badge-secondary'">
            {{ categoria.departamento_destino === 'HELPDESK' ? 'HELPDESK' : 'DATA' }}
          </span>
        </div>
        <div class="categoria-card-footer">
          <span class="categoria-date">📅 {{ formatDate(categoria.created_at) }}</span>
          <button class="btn btn-danger-link btn-sm" @click="confirmDelete(categoria)">
            🗑️ Eliminar
          </button>
        </div>
      ...
      <div v-if="categorias.length === 0" class="card empty-card">
        <p>No hay categorías registradas.</p>
      </div>
    </div>

    <!-- Modal Formulario de Nueva Categoría -->
    <div v-if="showForm" class="modal-overlay" @click.self="showForm = false">
      <div class="modal-content card">
        ...
        <form @submit.prevent="handleCreateCategoria" class="form-wrapper">
          <div class="modal-scroll-body">
            <div v-if="errorMsg" class="alert-error">{{ errorMsg }}</div>

            <div class="form-group">
              <label>Nombre de la Categoría / Caso</label>
              <input 
                type="text" 
                v-model="form.nombre" 
                class="form-control" 
                placeholder="Ej: Facturación Electrónica / Migración de Datos" 
                required 
              />
            </div>
    ...
    <div v-if="categoriaToDelete" class="modal-overlay" @click.self="categoriaToDelete = null">
      <div class="modal-content card" style="max-width: 440px;">
        <div class="modal-header">
          <h2 style="color: var(--danger-color);">Confirmar Eliminación</h2>
          <button class="close-btn" @click="categoriaToDelete = null">&times;</button>
        </div>
        <div style="padding: 0.5rem 0;">
          <p style="margin: 0; color: var(--text-main);">
            ¿Está seguro de que desea eliminar la categoría <strong>"{{ categoriaToDelete.nombre }}"</strong>?
          </p>
        </div>
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" @click="categoriaToDelete = null">Cancelar</button>
...
const fetchCategorias = async () => {
  try {
    categorias.value = await categoriaService.getCategorias(filterDepto.value)
  } catch (err) {
...
const handleCreateCategoria = async () => {
  try {
    ...
    errorMsg.value = err.message || 'Error al guardar la categoría'
  } finally {
    loading.value = false
  }
}
...
const executeDelete = async () => {
  ...
  try {
    loading.value = true
    await categoriaService.deleteCategoria(categoriaToDelete.value.id)
...
.mobile-categorias-list { display: flex; flex-direction: column; gap: 0.75rem; }
.categoria-card { padding: 1rem; }
.categoria-card-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.6rem; }
.categoria-name { font-size: 1.05rem; color: var(--text-main); }
.categoria-card-footer { display: flex; justify-content: space-between; align-items: center; padding-top: 0.6rem; border-top: 1px solid var(--border-color); }
.categoria-date { font-size: 0.8rem; color: var(--text-muted); }
